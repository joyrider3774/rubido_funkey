...
CMainMenu* CMainMenu_Create()
{
    ...
    Tmp = IMG_Load("./rubidofs/graphics/titlescreen.png");
    Result->IMGTitle = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);

    Tmp = IMG_Load("./rubidofs/graphics/newgame1.png");
    Result->IMGNewGame1 = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);

    Tmp = IMG_Load("./rubidofs/graphics/newgame2.png");
    Result->IMGNewGame2 = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);

    Tmp = IMG_Load("./rubidofs/graphics/credits1.png");
    Result->IMGCredits1 = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);

    Tmp = IMG_Load("./rubidofs/graphics/credits2.png");
    Result->IMGCredits2 = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);

	Tmp = IMG_Load("./rubidofs/graphics/quit1.png");
    Result->IMGQuit1 = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);

    Tmp = IMG_Load("./rubidofs/graphics/quit2.png");
    Result->IMGQuit2 = SDL_DisplayFormatAlpha(Tmp);
    SDL_FreeSurface(Tmp);
...
void CMainMenu_Destroy(CMainMenu* MainMenu)
{
	...
	SDL_FreeSurface(MainMenu->IMGQuit1);
    SDL_FreeSurface(MainMenu->IMGQuit2);
...
void CMainMenu_Draw(CMainMenu* MainMenu, SDL_Surface *Surface)
{
    ...
    {
        Dest.x = Surface->w/2 - MainMenu->IMGQuit1->w/2;
        Dest.y = 160;
        Dest.w = MainMenu->IMGQuit1->w;
        SDL_BlitSurface(MainMenu->IMGQuit1,NULL,Surface,&Dest);
    }
    else // blue
    {
        Dest.x = Surface->w/2 - MainMenu->IMGQuit2->w/2;
        Dest.y = 160;
        Dest.w = MainMenu->IMGQuit2->w;
        SDL_BlitSurface(MainMenu->IMGQuit2,NULL,Surface,&Dest);
    }
}
